...
describe("computeCommonName", () => {
  ...
  it("computes individual parts", () => {
    ...
    expect(computeCommonName(['2.1', '2.8', '3.1', '4.1'])).toEqual('2.1/2.8/3.1/4.1');
  });

  it("computes correctly between quotes", () => {
    expect(computeCommonName(['"Quad Core"', '"Eigth Core"'])).toEqual('"Quad/Eigth Core"');
  });

  it("computes complete models", () => {
    expect(computeCommonName([
    ...
    ])).toEqual('Mac Pro "Quad/Six/Eight/Twelve Core" 2.8/3.2/3.33/2.4/2.66/2.93/3.06 (2010/2012)');
    ...
    ])).toEqual('iMac Pro "8/10/14/18 Core" 3.2/3.0/2.5/2.3 27 Inch (5K, Late 2017)');
  });
});
